...
#   PriceChartDocumentData object that are relevant to US Steel
#   chart.
#
##############################################################################
...
def processPCDD(pcdd, tag):
    ...
    if False:
        # rluu's conclusions for this test on stock US Steel on 20120811:
        #
        # For velocity, I will want to watch the very extreme Mercury
        # speeds (either extreme high positive speed, or extreme high
        # negative speed).  Other points to watch are Mercury
        # retrograde points and direct points.  Sometimes, the extreme
        # high positive speed (only when greater than 2 times the Sun
        # speed) corresponds with lows, and sometimes the extreme high
        # negative speed (only when velocity is less than -1 times the
        # Sun speed) corresponds with highs.  The timing wasn't exact;
        # it was kind of fuzzy.
        #
        # Other planets did not show reliable results.
        #
        success = PlanetaryCombinationsLibrary.\
                  addGeoLongitudeVelocityLinesNonScaled(\
            pcdd, startDt, endDt, highPrice, lowPrice,
            planetName="Mercury", 
            color=None, stepSizeTd=stepSizeTd)
        success = PlanetaryCombinationsLibrary.\
    ...
    if False:
        # rluu 20120811 for US Steel:
        # Mercury's declination pivots frequently mark turns.
        # Other planets did not have a noticable effect.
