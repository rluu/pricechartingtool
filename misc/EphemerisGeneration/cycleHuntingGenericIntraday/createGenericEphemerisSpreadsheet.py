...
locationLongitude = -87.6277777777
locationLatitude = 41.8819444444
locationElevation = 0
...
endDt   = datetime.datetime(year=2014, month=12, day=31,
                            hour=hourOfDay, minute=minuteOfHour,
                            tzinfo=timezone)
#endDt   = datetime.datetime(year=2012, month=2, day=1,
#                            hour=hourOfDay, minute=minuteOfHour,
#                            tzinfo=timezone)
...
stepSizeTd = datetime.timedelta(minutes=15)
...
while currDt < endDt:
    ...
    # Only keep the same time of day if we are doing 1-day increments.  
    if stepSizeTd == datetime.timedelta(days=1):
        currDt = currDt.replace(hour=hourOfDay, minute=minuteOfHour)
